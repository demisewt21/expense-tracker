...
import { addTransaction } from '../../services/transactionService';   // ←←← FIXED PATH (two dots up)
...
const ExpenseForm = () => {
  ...
  // Backend category_id mapping (matches your database exactly)
  const categoryMap = {
    'Food': 1,
    'Transport': 3,
    'Rent': 2,
      ...
      <div className="bg-zinc-900 border border-white/10 rounded-3xl p-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-emerald-500 hover:bg-emerald-600 active:scale-[0.985] transition-all py-6 rounded-2xl font-semibold text-lg flex items-center justify-center gap-x-3 shadow-xl shadow-emerald-500/30 disabled:opacity-70"
          >
            {loading ? 'Saving...' : (
              <>
